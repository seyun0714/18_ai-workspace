...
/*
  ...
  2. 음성 입력 시작시 (이벤트 핸들러)
    1) SpeechRecognition 인서튼서 생성 여부, 브라우저 지원 여부 체크
    2) SpeechRecongtion 객체 스타트
  3. 음성 입력 종료 시 (이벤트 핸들러)
    1) SpeechRecognition 인스턴스 존재 여부 체크
    2) SpeechRecongtion 객체 스탑
*/
...
export default function STT_WebSpeechAPI() {
  ...
  useEffect(() => {
    ...
    // 브라우저 호환성 체크
    // - window 객체에 내장되어있는 SpeechRecongnition 생성자 찾기 (브라우저마다 이름이 다름)
    // - 찾아지지 않을 경우 === 지원이 되지 않음
    const SpeechRecognition =
      (window as any).SpeechRecongnition ||
      (window as any).webkitSpeechRecognition;
    if (SpeechRecognition) {
      setHasSTTSupport(true);

      // 음성 인식을 위한 인스턴스 생성 및 설정
      const recognition = new SpeechRecognition();
      recognition.lang = 'ko-KR'; // 인식 언어 설정
      recognition.continuous = true; // false: 단일 문장 인식 / true: 연속 인식
      recognition.interimResults = false; // false: 최종 결과만 / true: 중간 결과와 최종 결과 같이
  ...
  const handleStartListening = () => {
    if (hasSTTSupport && recognitionRef.current) {
  ...
  return (
    <section className="flex flex-col rounded-xl border border-zinc-200 bg-zinc-50 p-4">
      ...
      <p className="mb-3 text-xs text-zinc-600">
        마이크 버튼을 눌러 말해 보세요. <code>onresult</code>와{' '}
        <code>onend</code> 이벤트를 통해 결과를 처리합니다.
      </p>
