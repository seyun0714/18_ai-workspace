...
export default function SentimentAnalyzer() {
  ...
  const translaterRef = useRef<any>(null);
  ...
  useEffect(() => {
      ...
      try {
        const classifier = await pipeline('sentiment-analysis', undefined, {
          progress_callback: (args: any) => {
            if (args.status === 'done' || args.status === 'ready') return;
            setProgress(args.progress);
            setProgressLabel(`${args.file} 다운로드 중...`);
          },
        });
    ...
    async function loadTranslaterPipeline() {
      try {
        const translater = await pipeline(
          'translation',
        ...
        translaterRef.current = translater;
      } catch (error) {
        setError(error instanceof Error ? error.message : '모델 로드 실패');
        setLoading(false);
      }
    ...
    async function downloadAll() {
      await Promise.all([loadClassifierPipeline(), loadTranslaterPipeline()]);
      setProgress(100);
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const translaterPipe = translaterRef.current;
      if (!classifierPipe || !translaterPipe)
        throw new Error('모델이 로딩되지 않았습니다.');
      if (!input.trim()) throw new Error('분석할 문장을 입력해주세요.');

      const result = await Promise.all([
        classifierPipe(input.trim()),
        translaterPipe(input.trim(), {
          src_lang: 'kor_Hang',
          tgt_lang: 'eng_Latn',
        }),
      ]);
      console.log(result);
      const flatted = result.flatMap((v) => v);
      const output = { ...flatted[0], ...flatted[1] };
      setResult(output);
    } catch (error) {
    ...
    <div className="w-full max-w-2xl rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
      ...
      {!loading && (
        <>
          ...
          {(analyzing || result) && (
            <div className="min-h-12 rounded-lg bg-zinc-100 p-4 dark:bg-zinc-800/50">
              {analyzing && (
              ...
              {!analyzing && result && (
                <>
                  ...
                  <div>
                    <span>번역 결과: {result.translation_text}</span>
                  </div>
                </>
              )}
            </div>
